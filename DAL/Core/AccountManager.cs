...
namespace DAL.Core
{
    public class AccountManager : IAccountManager
    {
        ...
        public AccountManager(
            ApplicationDbContextMongo context,
        ...
        {
            var user = await _context.Users.Find(u => u.Id == userId)
                .SingleOrDefaultAsync();

            if (user == null)
                return null;

            var userRoleIds = user.Roles.Select(r => r).ToList();

            var roles = await _context.Roles
                .Find(r => userRoleIds.Contains(r.Id)).ToListAsync();
         
            var rolesarr = roles.Select( r => r.Name).ToList().ToArray();

            return (user, rolesarr);
        }

        public async Task<List<(ApplicationUser User, string[] Roles)>> GetUsersAndRolesAsync(int page, int pageSize)
        {
            List<ApplicationUser> usersQuery = _context.Users.Find(r => 1 == 1).ToList();


            var users = usersQuery;

            var userRoleIds = users.SelectMany(u => u.Roles.Select(r => r)).ToList();

            var roles = await _context.Roles
                .Find(r => userRoleIds.Contains(r.Id))
                .ToListAsync();

            return users
                .Select(u => (u, roles.Where(r => u.Roles.Select(ur => ur).Contains(r.Id)).Select(r => r.Name).ToArray()))
                .ToList();
        ...
        {
            if (await _context.Orders.Find(o => o.CashierId == userId).AnyAsync())
                return false;
        ...
        {
            var filter = new BsonDocument(); // An empty filter matches all documents
           
            List<ApplicationRole> rolesQuery = _context.Roles.Find(filter).ToList();
            //.Include(r => r.Claims)
            // .Include(r => r.Users)
            // .AsSingleQuery();
            // .OrderBy(r => r.Name);
        ...
        Task<bool> IAccountManager.TestCanDeleteRoleAsync(string roleId)
        {
